...
def main():
    ...
    franka = scene.add_entity(
        gs.morphs.MJCF(file='xml/franka_emika_panda/panda.xml',quat  =(1,0,0,0), # we use w-x-y-z convention for quaternions,
                                       scale=1.0))
    ...
    bootstraped_pos = np.array([-0.376, -0.661, 0.402, -0.189, -0.089, 0.824, 0.526])
    # move to pre-grasp pose
    scene.draw_debug_sphere(bootstraped_pos[:3]+offset_carton, radius=0.05, color=(0, 0, 1, 0.5))

    qpos = franka.inverse_kinematics(
        link = end_effector,
        pos  = bootstraped_pos[:3]+offset_carton,#np.array([0.65, 0.0, 0.25]),
        quat = bootstraped_pos[3:] #np.array([0, 1, 0, 0]),
    )
