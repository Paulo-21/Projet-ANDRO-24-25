...
def generate_arc(sim_scene,ref_vect_for_arc, u, stop_angle=30, start_angle=0, num_points=100, center_point=0,
    ...
    for angle_rad in angles_rad:
        rotation_matrix = rotation_matrix_theta_around_axisU(u=u, theta_rad=angle_rad)
...
def rotation_matrix_theta_around_axisU(u,theta_rad):
    ux = u[0]
    uy= u[1]
    uz=u[2]
    c=np.cos(theta_rad)
...
def apply_rotation_arround_articulation(sim_scene, nbr_item_joint_studied=3, multi_thread=None, radius=0.02, start_angle=0, stop_angle=90, geometric_debug=True,direction="positive"):
    ...
    parent_link_quat_pyquat = from_genesis_quat_to_pyquat(genesis_quat=parent_link_quat)
    ...
    child_link_quat_pyquat = from_genesis_quat_to_pyquat(genesis_quat=child_link_quat)
    ...
    if radius==None:
        ...
        for theta_rad in test_lispace:
            rotation_matrix_start = rotation_matrix_theta_around_axisU(u=OA_vect, theta_rad=theta_rad)
            rotation_matrix_stop = rotation_matrix_theta_around_axisU(u=OA_vect, theta_rad=stop_angle_rad)
    ...
    x_list, y_list, z_list = generate_arc( sim_scene=sim_scene,
    ...
    if multi_thread != "GPU_parallel":
        ...
        for i in range(n_step):
            ...
            sim_scene.robot.control_dofs_force(torch_push, [joint_action_indx])
            sim_scene.scene.step()
...
def rotate_around_axis(sim_scene):
    index_object_articulation = 2
    difference_init_end_action_joint_value = sim_scene.apply_rotation_arround_articulation(
...
def from_pyquat_to_genesis(pyquat):
    genesis_quat = np.array([pyquat[3], pyquat[0], pyquat[1], pyquat[2]])
    return genesis_quat


def from_genesis_quat_to_pyquat(genesis_quat):
